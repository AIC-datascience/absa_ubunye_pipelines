...
flood_mod = load_task_module(
    "pipelines/flood/etl/flood_risk", "transformations_flood_risk"
)
COLUMN_MAPPING = flood_mod.COLUMN_MAPPING
_build_items = flood_mod._build_items
compute_flood_risk = flood_mod.compute_flood_risk
jba_batch_request = flood_mod.jba_batch_request
...
def _make_post(endpoint_payloads: Dict[str, Any]):
    ...
    def _post(url, headers=None, params=None, json=None, timeout=None, verify=None):
        ...
        batch_ids = {g["id"] for g in (json or {}).get("geometries", [])}
        if batch_ids:
            full = [row for row in full if row.get("id") in batch_ids]
...
def test_compute_flood_risk_merges_scores_and_depths_and_renames():
    geocoded = pd.DataFrame(
        [
            ...
            {"id": "c", "inputAddress": "3 C St", "lat": None, "lon": 28.1},
        ]
    )
    scores_payload = [
