...
from conftest import load_task_module

telematics_mod = load_task_module(
    "pipelines/telematics/etl/policy_device_mapping", "transformations_telematics"
)
OUTPUT_COLUMNS = telematics_mod.OUTPUT_COLUMNS
_correct_installation = telematics_mod._correct_installation
_derive_policy_features = telematics_mod._derive_policy_features
_imei_first_detection = telematics_mod._imei_first_detection
_latest_mi = telematics_mod._latest_mi
_latest_telematics = telematics_mod._latest_telematics
build_policy_device_mapping = telematics_mod.build_policy_device_mapping
...
MI_SCHEMA_DDL = (
    "source_data string, policy_no string, policy_number string, "
    "policy_inception_date timestamp, item_inception_date timestamp, "
    "cancelled_date timestamp, eff_from_date timestamp, eff_to_date timestamp, "
    "section_code string, cover_code string, item_description string, "
    "veh_make string, veh_model string, registration string, "
    "vin_number string, imei_number string, "
    "cover_premium double, cover_sum_insured double, cover_description string, "
    "policy_status string, cover_status string, "
    "lob_asset_id string, item_no_raw string, policy_cancellation_reason_code string, "
    "end_date timestamp, effective_from_date timestamp, effective_to_date timestamp, "
    "effective_year_month string, effective_year int, effective_quarter int, effective_month int, "
    "effective_year_quarter string, item_cancelled_date timestamp, "
    "premium_collected double, active_lob_asset_id string"
)
...
@pytest.mark.parametrize("columns", [OUTPUT_COLUMNS])
