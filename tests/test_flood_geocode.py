...
geocode_mod = load_task_module(
    "pipelines/flood/etl/geocode_addresses", "transformations_geocode"
)
batch_geocode = geocode_mod.batch_geocode
GEOCODE_OUTPUT_COLUMNS = geocode_mod.GEOCODE_OUTPUT_COLUMNS
...
class FakeSession:
    ...
    def get(self, url, params=None, timeout=None, verify=None):
        ...
        for addr, payload in self.payload_by_address.items():
            # ``_tomtom_call`` uses ``urllib.parse.quote(addr, safe="")`` so
            # both spaces and commas are escaped; match the fully quoted
            # form, as well as the raw string for defensiveness.
            quoted = addr.replace(" ", "%20").replace(",", "%2C")
            if quoted in url or addr in url:
...
@pytest.mark.parametrize("cols", [GEOCODE_OUTPUT_COLUMNS])
